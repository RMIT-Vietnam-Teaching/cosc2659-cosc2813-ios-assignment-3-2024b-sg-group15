...
struct MapComponent5: View {
                        ...
                        ForEach(mapPoints, id: \.name) { point in
                            Button(action: {
                                ...
                                Text(point.name) // Display the actual place name
                                    ...
                                    .foregroundColor(selectedButton == point.name ? .yellow : .black)
                                    .cornerRadius(10)
    ...
    func customButtonWidth(for size: CGSize, multiplier: CGFloat, landscapeMultiplierCompact: CGFloat, landscapeMultiplierRegular: CGFloat) -> CGFloat {
        ...
        if verticalSizeClass == .compact {
            // Landscape mode
            baseWidth = horizontalSizeClass == .compact ? size.width * 0.028 * landscapeMultiplierCompact : size.width * 0.08 * landscapeMultiplierRegular
        } else {
            // Portrait mode
            baseWidth = horizontalSizeClass == .compact ? size.width * 0.12 : size.width * 0.14
        }
    ...
    func customButtonHeight(for size: CGSize, multiplier: CGFloat, landscapeMultiplierCompact: CGFloat, landscapeMultiplierRegular: CGFloat) -> CGFloat {
        ...
        if verticalSizeClass == .compact {
            // Landscape mode
            baseHeight = horizontalSizeClass == .compact ? size.height * 0.04 * landscapeMultiplierCompact : size.height * 0.05 * landscapeMultiplierRegular
        } else {
            // Portrait mode
            baseHeight = horizontalSizeClass == .compact ? size.height * 0.03 : size.height * 0.05
        }
    ...
    func dynamicTextSize() -> CGFloat {
          if verticalSizeClass == .compact {
              // Landscape mode
              return horizontalSizeClass == .compact ? 21 : 40
          } else {
              // Portrait mode
              return horizontalSizeClass == .compact ? 17 : 43
          }
      }
}
